...
def load_models():
    ...
    for style_key, style_info in BEST_MODELS.items():
        ...
        try:
            ...
            generator_wrapper = SG2Generator(base_model_path, img_size=1024, device='cpu')
...
if page == "🎨 Генерация":
                ...
                try:
                    ...
                    with torch.no_grad():
                        for i in range(num_images):
                            ...
                            image, _ = generator([z], truncation=truncation, truncation_latent=truncation_latent)
    ...
    if not os.path.exists(visualizations_dir):
        ...
        with col2:
            ...
            convergence_best = os.path.join(visualizations_dir, "convergence_best_models.png")
        ...
        if os.path.exists(convergence_dir):
            convergence_files = [f for f in os.listdir(convergence_dir) if f.endswith('.png')]
    ...
    if os.path.exists(metrics_file):
            ...
            for model_name, model_info in models_data.items():
                if model_info.get('exists'):
                    rows.append({
                        ...
                        'Финальный Loss': f"{model_info.get('final_loss', 0):.4f}" if model_info.get('final_loss') else 'N/A',
                        'Cosine Similarity': f"{model_info.get('cos_sim', 0):.4f}" if model_info.get('cos_sim') else 'N/A',
                        'Размер (MB)': f"{model_info.get('size_mb', 0):.2f}" if model_info.get('size_mb') else 'N/A'
                    })
        ...
        if stats:
            ...
            with col1:
                st.metric("Средний Loss", f"{stats.get('avg_loss', 0):.4f}" if stats.get('avg_loss') else 'N/A')
            with col2:
                st.metric("Средний Cosine Similarity", f"{stats.get('avg_cos_sim', 0):.4f}" if stats.get('avg_cos_sim') else 'N/A')
            with col3:
                st.metric("Общий размер моделей (MB)", f"{stats.get('total_size_mb', 0):.2f}" if stats.get('total_size_mb') else 'N/A')
        
        st.subheader("Лучшие модели по стилям")
        best_models = metrics_data.get('summary', {}).get('best_models_by_style', {})
        if best_models:
            for style, info in best_models.items():
                st.write(f"**{style}**: {info.get('model_name', 'N/A')} (cos_sim: {info.get('cos_sim', 0):.4f})")
