...
def check_cpu_device():
    """Проверяет, используется ли CPU"""
    return not torch.cuda.is_available()


def apply_cpu_support_to_fused_act(file_path):
    """Применяет CPU-поддержку к fused_act.py"""
...
def apply_cpu_support_to_upfirdn2d(file_path):
    """Применяет CPU-поддержку к upfirdn2d.py"""
    print(f"Применение CPU-поддержки к {file_path}...")
...
if torch.cuda.is_available():
    ...
    # Обновляем UpFirDn2dBackward.forward - нужно найти правильный блок
    # Ищем начало функции forward в UpFirDn2dBackward
...
def setup_cpu_support(stylegan_nada_dir=None):
    """
    Применяет CPU-поддержку к файлам stylegan-nada.
    
    Args:
        stylegan_nada_dir: Путь к директории stylegan_nada. Если None, ищет в стандартных местах.
    """
    ...
    # Определяем путь к stylegan_nada
    if stylegan_nada_dir is None:
        ...
        for path in possible_paths:
            ...
            if os.path.exists(abs_path) and os.path.exists(os.path.join(abs_path, "ZSSGAN")):
    ...
    try:
        ...
        print(f"\n✗ Ошибка при применении CPU-поддержки: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Применяет CPU-поддержку к stylegan-nada")
    parser.add_argument(
        "--stylegan-nada-dir",
        type=str,
        default=None,
