...
│   ├── inference_output          # Результаты инференса (изображения)
│   ├── train_and_save.ipynb      # Обучение моделей
│   └── inference.ipynb           # Инференс (генерация случайных изображений)
├── web_app/                      # Streamlit приложение
│   └── app.py                    # Главный файл приложения 
├── models/                       # Предобученные модели
│   ├── ffhq.pt                   # Базовая модель StyleGAN2 FFHQ
│   └── final_model_*.pt          # Обученные модели
├── output/                       # Результаты обучения
│   ├── {style}_freeze_{k}/       # Директории для каждого эксперимента
│   │   ├── training_history.json # История обучения
│   │   ├── convergence_plot.png  # График сходимости
│   │   └── sample/               # Примеры генераций
│   └── visualizations/           # Визуализации результатов
├── stylegan_nada/                # Клонированный репозиторий StyleGAN-NADA
├── .streamlit/                   # Конфигурация Streamlit
│   └── config.toml
├── .gitattributes                # Конфигурация Git LFS для больших файлов
├── requirements.txt              # Зависимости для локальной разработки
├── packages.txt                  # Зависимости для streamlit cloud (деплой через GitHub)
└── README.md                     
```
...
2. Инференс (генерация случайных изображений) реализован в `notebooks/inference.ipynb`. Результаты сохраняются в `notebooks/inference_output`

### Этап 3: Создание веб-приложения

Веб-приложение на Streamlit включает три раздела:
- **Генерация**: интерактивная генерация изображений в различных стилях с настройкой параметров (seed, truncation)
...
Приложение использует только лучшие модели по каждому стилю (определены в README) и автоматически загружает базовую модель FFHQ из облака.
